(() => {
  ...
  function dashboardSvg(values, labels, metric, type='bar', expanded=false, statusFlags=[]) {
    ...
    const axisLabels=labels.map((label,i)=>i%step===0?`<text x="${x(i)}" y="${H-10}" text-anchor="middle" fill="#718096" font-size="${expanded?11:10}" font-weight="${expanded?700:400}">${esc(label)}</text>`:'').join('');
    let marks='';
    if(type==='line'){
      ...
      const unit=(W-left-right)/Math.max(1,values.length),barWidth=Math.max(5,unit*.58),fontSize=veryDense?5.6:dense?6.2:10;
  ...
  function dashboardDualCompareSvg(rows, labels, expanded=false){
    ...
    const merc=rows.map(([,d])=>num(d.general)), serv=rows.map(([,d])=>num(d.warranty)+num(d.other)+num(d.mixed));
    const W=expanded?Math.max(980,Math.min(1780,140+rows.length*58)):780,H=expanded?410:330,left=74,right=24,top=60,bottom=44;
    const lo=Math.min(0,...merc,...serv),hi=Math.max(0,...merc,...serv),span=(hi-lo)||1,pad=span*.14,min=lo<0?lo-pad:0,max=hi+pad;
    const y=v=>top+(max-v)/(max-min)*(H-top-bottom),zeroY=y(0),unit=(W-left-right)/Math.max(1,rows.length),groupW=Math.min(unit*.76,46),gap=Math.max(2,groupW*.10),bw=(groupW-gap)/2;
    const ticks=5,tickVals=Array.from({length:ticks},(_,i)=>max-(max-min)*i/(ticks-1));
    const grid=tickVals.map(v=>{const yy=y(v);return `<line x1="${left}" y1="${yy}" x2="${W-right}" y2="${yy}" stroke="#e7edf4"/><text x="8" y="${yy+4}" fill="#718096" font-size="10">${dashboardAxisFormat(v,'merc')}</text>`}).join('');
    const step=expanded?1:Math.max(1,Math.ceil(labels.length/8)),labs=labels.map((l,i)=>i%step===0?`<text x="${left+(i+.5)*unit}" y="${H-12}" text-anchor="middle" fill="#718096" font-size="${expanded?11:10}" font-weight="${expanded?700:400}">${esc(l)}</text>`:'').join('');
    const bars=rows.map(([,d],i)=>{const pair=[merc[i],serv[i]],colors=[['#1688ec','#6550e8'],['#16a085','#5dd8c2']],cx=left+(i+.5)*unit,base=cx-groupW/2;return pair.map((v,j)=>{const xx=base+j*(bw+gap),yv=y(v),yy=Math.min(zeroY,yv),hh=Math.max(2,Math.abs(zeroY-yv)),labelY=v>=0?Math.max(top+12,yy-8):Math.min(H-bottom-4,yy+hh+14),tone=v<0?'#c9354d':'#17324d',gid=`bd-${i}-${j}-${expanded?1:0}`;return `<defs><linearGradient id="${gid}" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="${colors[j][0]}"/><stop offset="1" stop-color="${colors[j][1]}"/></linearGradient></defs><rect x="${xx}" y="${yy}" width="${bw}" height="${hh}" rx="4" fill="url(#${gid})"/><text x="${xx+bw/2}" y="${labelY}" text-anchor="middle" fill="${tone}" font-size="${expanded?8:6.2}" font-weight="800">${brl.format(v)}</text>`}).join('')}).join('');
    return `<div class="dual-chart-legend"><span><i class="merc"></i>Venda mercantil</span><span><i class="serv"></i>Serviços</span></div><svg class="svg-chart dual-compare-svg" viewBox="0 0 ${W} ${H}" role="img">${grid}<line x1="${left}" y1="${zeroY}" x2="${W-right}" y2="${zeroY}" stroke="#9aa9ba" stroke-width="1.4"/>${bars}${labs}</svg>`;
  }
  function dashboardMercEfficiencyTrendSvg(rows, labels, expanded=false){
    ...
    const W=expanded?Math.max(980,Math.min(1780,140+rows.length*58)):780,H=expanded?410:330,left=72,right=72,top=60,bottom=44;
    ...
    const path=(arr,fn)=>arr.map((v,i)=>`${x(i)},${fn(v)}`).join(' '),step=expanded?1:Math.max(1,Math.ceil(labels.length/8)),labs=labels.map((l,i)=>i%step===0?`<text x="${x(i)}" y="${H-12}" text-anchor="middle" fill="#718096" font-size="${expanded?11:10}">${esc(l)}</text>`:'').join('');
    const mp=merc.map((v,i)=>`<circle cx="${x(i)}" cy="${ym(v)}" r="3.5" fill="#1688ec"><title>${esc(labels[i])} • Mercantil ${brl.format(v)}</title></circle>`).join(''),ep=eff.map((v,i)=>`<circle cx="${x(i)}" cy="${ye(v)}" r="3.5" fill="#f59e0b"><title>${esc(labels[i])} • Eficiência ${v.toFixed(2).replace('.',',')}%</title></circle>`).join('');
  ...
  function dashboardMercServicesCompareSection(rows){
    const multiMonth=new Set(rows.map(([k])=>String(k).slice(0,7))).size>1,labels=rows.map(([k])=>multiMonth?`${String(k).slice(8,10)}/${String(k).slice(5,7)}`:String(Number(String(k).slice(-2))));
    return `<section class="dashboard-section branch-theme-mercservices"><div class="dashboard-section-head"><div><h3>📊 Mercantil × Serviços e eficiência</h3><p>Leitura comparativa adicional, sem alterar os gráficos originais.</p></div></div><div class="dashboard-chart-grid"><div class="dashboard-card dual-chart-card"><button class="dashboard-chart-expand" type="button" data-chart-expand aria-label="Ampliar gráfico">⛶</button><h4>Mercantil × Serviços por dia</h4><div class="hint">Barras lado a lado; valores negativos descem abaixo da linha zero.</div>${dashboardDualCompareSvg(rows,labels,false)}</div><div class="dashboard-card dual-chart-card"><button class="dashboard-chart-expand" type="button" data-chart-expand aria-label="Ampliar gráfico">⛶</button><h4>Tendência: Mercantil × Eficiência</h4><div class="hint">Mercantil no eixo esquerdo; eficiência no eixo direito.</div>${dashboardMercEfficiencyTrendSvg(rows,labels,false)}</div></div></section>`;
  }
    ...
    if(type!=='revenue'){
      return amounts.map((a,index)=>{
        ...
        if(biIsPaymentType(type)){
          ...
          if(!label && amounts.length<=5){
            const paymentFallback=['Carnê','PIX','Cartão de crédito','Dinheiro','Cartão de débito'];
            label=paymentFallback[index]||'';
          }
