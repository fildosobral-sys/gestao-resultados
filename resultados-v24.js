(() => {
  ...
  function sellerFinancials(seller, source = db) {
    ...
    const effectiveMercantileRate = num(seller.general) ? mercantileCommission / num(seller.general) : legacyMercRate;
    const serviceRate = services ? serviceCommission / services : 0;
    ...
    return { services, serviceCommission, mercantileCommission, commissionSubtotal, plannedDays, calendarDays, sundayCount, automaticRestDays, restDays, justifiedDays, paidDays, workedDays, dsr, total, projectedSubtotal, projectedDsr, projectedTotal, mercantileRate: effectiveMercantileRate, serviceRate, targetMercantileCommission, targetServiceCommission, targetSubtotal, targetPaid, targetTotal };
  }
  ...
  function sellerWorkspaceWeeks(seller) {
    const rows = Object.entries(seller.daily || {}).sort(([a],[b]) => a.localeCompare(b));
    const groups = new Map();
    rows.forEach(([key, day]) => { const d=new Date(`${key}T12:00:00`); d.setDate(d.getDate()-((d.getDay()+6)%7)); const wk=isoDate(d.getFullYear(),d.getMonth()+1,d.getDate()); if(!groups.has(wk))groups.set(wk,[]); groups.get(wk).push([key,day]); });
    return [...groups.entries()].map(([key,items]) => { const tmp={daily:Object.fromEntries(items)}, a=sellerDailyAggregate(tmp); return {key,items,a}; });
  }
  function sellerWorkspacePeriodCard(label, a, mercGoal, serviceGoal) {
    return `<div class="seller-workspace-card"><span>${label}</span><strong>${brl.format(a.general)}</strong><small>Serviços ${brl.format(a.services)} • Conv. ${a.nfs?efficiencyPct.format(a.conversion):'—'} • Efic. ${a.eligible?efficiencyPct.format(a.efficiency):'—'}</small></div>`;
  }
  ...
  function sellerDashboardMarkup(seller, period='month', metric='merc', compare='prev') {
    if(period==='year'){
      const history=sellerProfileHistory(seller).slice().sort((a,b)=>a.month.localeCompare(b.month)); const currentYear=Number(String(db.month).slice(0,4)), current=history.filter(h=>Number(h.month.slice(0,4))===currentYear), prev=history.filter(h=>Number(h.month.slice(0,4))===currentYear-1); const vals=current.map(h=>metric==='services'?h.services:metric==='gain'?h.financial.total:num(h.seller.general)+num(h.seller.ecommerce)), labs=current.map(h=>monthLabel(h.month).slice(0,3)); const prevTotal=prev.reduce((a,h)=>a+(metric==='services'?h.services:metric==='gain'?h.financial.total:num(h.seller.general)+num(h.seller.ecommerce)),0), curTotal=vals.reduce((a,b)=>a+b,0), delta=prevTotal?(curTotal-prevTotal)/prevTotal:0;return `<div class="seller-dashboard-shell"><div class="dashboard-controls"><div><label>Período</label><select data-dash-period><option value="week">Semana</option><option value="fortnight">15 dias</option><option value="month">Mês</option><option value="year" selected>Ano</option></select></div><div><label>Indicador</label><select data-dash-metric>${[['merc','Mercantil'],['services','Serviços'],['gain','Ganhos']].map(([v,l])=>`<option value="${v}" ${metric===v?'selected':''}>${l}</option>`).join('')}</select></div><div><label>Comparação</label><select data-dash-compare><option value="prev" ${compare==='prev'?'selected':''}>Ano anterior</option><option value="none" ${compare==='none'?'selected':''}>Somente atual</option></select></div></div><div class="dashboard-kpis"><div class="dashboard-kpi"><span>Total do ano</span><strong>${dashboardFormat(curTotal,metric)}</strong></div><div class="dashboard-kpi"><span>Ano anterior</span><strong>${compare==='prev'&&prevTotal?dashboardFormat(prevTotal,metric):'—'}</strong></div><div class="dashboard-kpi"><span>Evolução</span><strong class="${delta>0?'trend-up':delta<0?'trend-down':'trend-flat'}">${prevTotal?(delta>=0?'▲ ':'▼ ')+Math.abs(delta*100).toFixed(1).replace('.',',')+'%':'—'}</strong></div><div class="dashboard-kpi"><span>Meses com dados</span><strong>${current.length}</strong></div></div><div class="dashboard-card"><h4>${dashboardLabel(metric)} · mês a mês</h4><div class="hint">Histórico disponível na nuvem para ${currentYear}.</div>${dashboardSvg(vals,labs,metric,'bar')}</div></div>`;
    }
    const rows=sellerDashboardPeriodRows(seller,period), vals=rows.map(([,d])=>dashboardMetricValue(d,metric)), labs=rows.map(([k])=>String(Number(k.slice(-2)))), agg=sellerDashboardAggregateRows(rows), total=metric==='services'?agg.services:metric==='conversion'?(agg.nfs?agg.warrantyQty/agg.nfs*100:0):metric==='efficiency'?(agg.eligible?agg.services/agg.eligible*100:0):metric==='ticket'?(agg.invoiceCount?agg.general/agg.invoiceCount:0):metric==='gain'?rows.reduce((a,[,d])=>a+dashboardMetricValue(d,'gain'),0):agg.general; const nonzero=vals.filter(v=>v>0), best=nonzero.length?Math.max(...nonzero):0, worst=nonzero.length?Math.min(...nonzero):0; const half=Math.max(1,Math.floor(vals.length/2)), first=vals.slice(0,half).reduce((a,b)=>a+b,0), second=vals.slice(half).reduce((a,b)=>a+b,0), trend=first?((second/(Math.max(1,vals.length-half)))-(first/half))/(first/half):0; const w=num(agg.warranty),m=num(agg.mixed),o=num(agg.other),mixTotal=w+m+o||1, pie=`conic-gradient(#668ce8 0 ${w/mixTotal*100}%,#57d6c5 ${w/mixTotal*100}% ${(w+m)/mixTotal*100}%,#f5b84f ${(w+m)/mixTotal*100}% 100%)`;
    return `<div class="seller-dashboard-shell"><div class="dashboard-controls"><div><label>Período</label><select data-dash-period>${[['week','Semana'],['fortnight','15 dias'],['month','Mês'],['year','Ano']].map(([v,l])=>`<option value="${v}" ${period===v?'selected':''}>${l}</option>`).join('')}</select></div><div><label>Indicador</label><select data-dash-metric>${[['merc','Mercantil'],['services','Serviços'],['conversion','Conversão'],['efficiency','Eficiência'],['ticket','Ticket médio'],['gain','Ganhos']].map(([v,l])=>`<option value="${v}" ${metric===v?'selected':''}>${l}</option>`).join('')}</select></div><div><label>Comparação</label><select data-dash-compare><option value="prev" ${compare==='prev'?'selected':''}>Período anterior</option><option value="none" ${compare==='none'?'selected':''}>Somente atual</option></select></div></div><div class="dashboard-kpis"><div class="dashboard-kpi"><span>Resultado</span><strong>${dashboardFormat(total,metric)}</strong></div><div class="dashboard-kpi"><span>Melhor dia</span><strong>${dashboardFormat(best,metric)}</strong></div><div class="dashboard-kpi"><span>Menor dia</span><strong>${dashboardFormat(worst,metric)}</strong></div><div class="dashboard-kpi"><span>Tendência</span><strong class="${trend>0.02?'trend-up':trend<-.02?'trend-down':'trend-flat'}">${trend>0.02?'▲ Ascendente':trend<-.02?'▼ Descendente':'→ Estável'}</strong></div></div><div class="dashboard-chart-grid"><div class="dashboard-card"><h4>${dashboardLabel(metric)} por dia</h4><div class="hint">Barras para identificar picos e dias de menor resultado.</div>${dashboardSvg(vals,labs,metric,'bar')}</div><div class="dashboard-card"><h4>Tendência</h4><div class="hint">Leitura visual do comportamento no período.</div>${dashboardSvg(vals,labs,metric,'line')}</div></div><div class="dashboard-chart-grid" style="margin-top:10px"><div class="dashboard-card"><h4>Composição dos serviços</h4><div class="hint">Garantia, presta-mista e outros serviços.</div><div class="dashboard-pie-wrap"><div class="dashboard-pie" style="background:${pie}"></div><div class="dashboard-legend"><div><i style="background:#668ce8"></i><span>Garantia</span><strong>${brl.format(w)}</strong></div><div><i style="background:#57d6c5"></i><span>Presta-mista</span><strong>${brl.format(m)}</strong></div><div><i style="background:#f5b84f"></i><span>Outros</span><strong>${brl.format(o)}</strong></div></div></div></div><div class="dashboard-card"><h4>Indicadores do período</h4><div class="dashboard-kpis" style="grid-template-columns:1fr 1fr;margin:0"><div class="dashboard-kpi"><span>Conversão</span><strong>${agg.nfs?efficiencyPct.format(agg.conversion):'—'}</strong><small>Meta 35%</small></div><div class="dashboard-kpi"><span>Eficiência</span><strong>${agg.eligible?efficiencyPct.format(agg.efficiency):'—'}</strong><small>Meta 7%</small></div><div class="dashboard-kpi"><span>Ticket médio</span><strong>${agg.invoiceCount?brl.format(agg.general/agg.invoiceCount):'—'}</strong></div><div class="dashboard-kpi"><span>Dias com dados</span><strong>${rows.filter(([,d])=>hasSellerDayValue(d)).length}</strong></div></div></div></div></div>`;
  }
  function mountSellerDashboard(seller, root){ if(!root)return; let period=root.dataset.period||'month',metric=root.dataset.metric||'merc',compare=root.dataset.compare||'prev'; const draw=()=>{root.innerHTML=sellerDashboardMarkup(seller,period,metric,compare); const p=root.querySelector('[data-dash-period]'),m=root.querySelector('[data-dash-metric]'),c=root.querySelector('[data-dash-compare]'); if(p)p.onchange=()=>{period=p.value;root.dataset.period=period;draw()};if(m)m.onchange=()=>{metric=m.value;root.dataset.metric=metric;draw()};if(c)c.onchange=()=>{compare=c.value;root.dataset.compare=compare;draw()};};draw(); }
  function renderTeamDashboard(){const host=document.getElementById('teamDashboardHost');if(!host||host.hidden)return; const metric=host.dataset.metric||'merc',period=host.dataset.period||'month'; const rows=db.sellers.map((seller,index)=>{const pr=sellerDashboardPeriodRows(seller,period),a=sellerDashboardAggregateRows(pr),value=metric==='services'?a.services:metric==='conversion'?(a.nfs?a.conversion*100:0):metric==='efficiency'?(a.eligible?a.efficiency*100:0):a.general+(period==='month'?num(seller.ecommerce):0);return{name:seller.name||`Vendedor ${index+1}`,value}}); const max=Math.max(...rows.map(r=>r.value),1);host.innerHTML=`<div class="team-dashboard-title"><div><h3>📊 Dashboard da equipe</h3><div class="hint">Compare os vendedores no período escolhido.</div></div><div style="display:flex;gap:7px"><div class="field" style="min-width:125px"><select id="teamDashPeriod"><option value="week" ${period==='week'?'selected':''}>Semana</option><option value="fortnight" ${period==='fortnight'?'selected':''}>15 dias</option><option value="month" ${period==='month'?'selected':''}>Mês</option></select></div><div class="field" style="min-width:145px"><select id="teamDashMetric"><option value="merc" ${metric==='merc'?'selected':''}>Mercantil</option><option value="services" ${metric==='services'?'selected':''}>Serviços</option><option value="conversion" ${metric==='conversion'?'selected':''}>Conversão</option><option value="efficiency" ${metric==='efficiency'?'selected':''}>Eficiência</option></select></div></div></div><div class="team-seller-bars">${rows.sort((a,b)=>b.value-a.value).map(r=>`<div class="team-seller-bar"><b>${esc(r.name)}</b><div class="team-track"><i style="width:${Math.max(2,r.value/max*100)}%"></i></div><strong>${['conversion','efficiency'].includes(metric)?r.value.toFixed(1).replace('.',',')+'%':brl.format(r.value)}</strong></div>`).join('')||'<div class="empty">Sem vendedores cadastrados.</div>'}</div>`; const sel=host.querySelector('#teamDashMetric'),per=host.querySelector('#teamDashPeriod');if(sel)sel.onchange=()=>{host.dataset.metric=sel.value;renderTeamDashboard()};if(per)per.onchange=()=>{host.dataset.period=per.value;renderTeamDashboard()}; }
  function renderSellerWorkspace(seller) {
    const host=document.getElementById('sellerWorkspace'); if(!host)return;
    const total=sellerDailyAggregate(seller), metrics=sellerMetrics(seller), history=sellerProfileHistory(seller), days=Object.entries(seller.daily||{}).sort(([a],[b])=>b.localeCompare(a));
    ...
    const todayKey=isoDate(today.getFullYear(), today.getMonth()+1, today.getDate()), todayData=seller.daily?.[todayKey]; const hasToday=todayData&&['general','eligible','warranty','other','mixed','nfs','warrantyQty'].some(f=>num(todayData[f])>0); const launchedDays=days.filter(([,d])=>d.status==='done'||hasSellerDayValue(d)).length, pendingDays=days.filter(([k,d])=>k<=todayKey&&d.status!=='off'&&d.status!=='medical'&&d.status!=='justified'&&d.status!=='done'&&!hasSellerDayValue(d)).length;
    ...
    const statusTone=pendingDays?'attention':hasToday?'ok':'neutral';
    const overview=`<div class="seller-overview-strip"><div><span>📅 LANÇAMENTOS</span><strong>${launchedDays}</strong><small>dia(s) com resultado</small></div><div class="${statusTone}"><span>${pendingDays?'⚠️':'✅'} PENDÊNCIAS</span><strong>${pendingDays}</strong><small>${pendingDays?'requer acompanhamento':'nenhuma pendência registrada'}</small></div><div><span>☁️ SINCRONIZAÇÃO</span><strong>${hasToday?'Hoje':'Ativa'}</strong><small>${seller.updatedAt?new Date(seller.updatedAt).toLocaleString('pt-BR'):'aguardando atualização'}</small></div></div>
    <h3 class="seller-block-title">📊 Desempenho e projeção</h3><div class="seller-workspace-grid">${[
      ...
      ['📅 Dias considerados',String(consideredDays),`${remaining} restante(s) de ${planned}`],['🌐 E-commerce',brl.format(ecommerce),'Acumulado do mês']
    ...
    const daily=`<div class="seller-day-cards">${days.length?days.map(([key,day])=>{const services=num(day.warranty)+num(day.other)+num(day.mixed),conv=num(day.nfs)?num(day.warrantyQty)/num(day.nfs):0,eff=num(day.eligible)?services/num(day.eligible):0,mercComm=Object.prototype.hasOwnProperty.call(day,'commissionMercantile')?num(day.commissionMercantile):num(day.general)*num(day.commissionMercantileRate)/100,servComm=Object.prototype.hasOwnProperty.call(day,'commissionService')?num(day.commissionService):services*.05;const has=hasSellerDayValue(day);const st=day.status==='off'?'💤 Não trabalha':day.status==='medical'?'🩺 Atestado':day.status==='justified'?'📋 Justificada':day.status==='done'?'✅ Finalizado':has||day.status==='partial'?'🟡 Parcial':'⚠️ Pendente';return `<article class="seller-day-card"><div><span>DIA</span><strong>${new Date(`${key}T12:00:00`).toLocaleDateString('pt-BR')}</strong></div><div><span>STATUS</span><strong>${st}</strong></div><div><span>MERCANTIL</span><strong>${brl.format(num(day.general))}</strong></div><div><span>SERVIÇOS</span><strong>${brl.format(services)}</strong></div><div><span>CONVERSÃO</span><strong>${num(day.nfs)?efficiencyPct.format(conv):'—'}</strong></div><div><span>EFICIÊNCIA</span><strong>${num(day.eligible)?efficiencyPct.format(eff):'—'}</strong></div><div><span>COMISSÃO MERC.</span><strong>${brl.format(mercComm)}</strong><small>valor informado</small></div><div><span>COMISSÃO SERVIÇOS</span><strong>${brl.format(servComm)}</strong><small>valor informado</small></div></article>`}).join(''):'<div class="empty">Nenhum lançamento recebido deste vendedor.</div>'}</div>`;
    const weeks=sellerWorkspaceWeeks(seller); const weekly=`<div class="seller-workspace-grid">${weeks.length?weeks.map((w,i)=>sellerWorkspacePeriodCard(`${i+1}ª semana`,w.a,0,0)).join(''):'<div class="empty">Sem resultados semanais ainda.</div>'}</div>`;
  ...
  function sellerDailyStatus(seller) {
    ...
    const day = seller.daily?.[key];
    const hasAny = day && ['general','eligible','warranty','other','mixed','nfs','warrantyQty'].some((field) => num(day[field]) > 0);
    if (day?.status === 'done' || hasAny) return { label:'Atualizado hoje', cls:'ok' };
  ...
  function renderSellers() {
    ...
    list.innerHTML = db.sellers.map((seller, index) => {
      ...
      return `<article class="seller-directory-card" data-i="${index}">
        <div class="seller-person"><strong>${esc(seller.name || `Vendedor ${index + 1}`)}</strong><span>${metrics.plannedDays} dias planejados • ${daily.days} lançados • ${metrics.remainingDays} restantes</span></div>
        ...
        <div class="seller-card-footer"><div class="seller-card-status"><span class="seller-pending ${status.cls}">${status.label}</span><small class="seller-updated"><span class="updated-full">${seller.updatedAt?'🕐 '+new Date(seller.updatedAt).toLocaleString('pt-BR'):'🕐 Sem atualização'}</span><span class="updated-short">${seller.updatedAt?'🕐 '+new Date(seller.updatedAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'🕐 —'}</span></small></div>
    ...
    const teamBtn=document.getElementById('teamDashboardBtn'); if(teamBtn&&!teamBtn.dataset.bound){teamBtn.dataset.bound='1';teamBtn.addEventListener('click',()=>{const host=document.getElementById('teamDashboardHost');host.hidden=!host.hidden;teamBtn.textContent=host.hidden?'📊 Dashboard equipe':'✕ Fechar dashboard';renderTeamDashboard();});}
    renderTeamDashboard();
  }
