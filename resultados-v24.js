  ...
  function renderSellers() {
    ...
    list.innerHTML = db.sellers.map((seller, index) => {
      ...
      return `<article class="seller-directory-card" data-i="${index}">
        ...
        <div class="seller-card-footer"><div class="seller-card-status"><span class="seller-pending ${status.cls}">${status.label}</span><small>${seller.updatedAt?'🕐 '+new Date(seller.updatedAt).toLocaleString('pt-BR'):'🕐 Sem atualização'}</small></div>
        <div class="seller-directory-actions"><button type="button" class="btn primary small seller-open-btn" data-open-seller="${index}">👁️ Ver vendedor</button><button type="button" class="btn secondary small seller-edit-btn" data-edit-seller="${index}">✏️ Editar</button><button type="button" class="btn danger small seller-delete-btn" data-delete-seller="${index}">🗑️ Excluir</button></div></div>
      </article>`;
    }).join('');
