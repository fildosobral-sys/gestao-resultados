(() => {
  ...
  function dashboardDualCompareSvg(rows, labels, expanded=false){
    ...
    const step=expanded?1:Math.max(1,Math.ceil(labels.length/8));
    const labs=labels.map((l,i)=>`<text class="dual-x-label ${i%step===0?'':'dual-x-compact'}" x="${left+(i+.5)*unit}" y="${H-12}" text-anchor="middle" fill="#718096" font-size="${expanded?11:10}" font-weight="${expanded?700:400}">${esc(l)}</text>`).join('');
    const bars=rows.map(([,d],i)=>{
      ...
      return values.map((v,j)=>{
        ...
        return `<defs><linearGradient id="${gid}" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="${colors[j][0]}"/><stop offset="1" stop-color="${colors[j][1]}"/></linearGradient></defs><rect x="${xx}" y="${yy}" width="${bw}" height="${hh}" rx="4" fill="url(#${gid})"/><text x="${xx+bw/2}" y="${labelY}" text-anchor="middle" fill="${tone}" font-size="${expanded?9:6.4}" font-weight="900">${txt}</text>`;
      }).join('');
    }).join('');
  ...
  function dashboardMercEfficiencyTrendSvg(rows, labels, expanded=false){
    ...
    const labs=labels.map((l,i)=>`<text class="dual-x-label ${i%step===0?'':'dual-x-compact'}" x="${x(i)}" y="${H-12}" text-anchor="middle" fill="#718096" font-size="${expanded?11:10}">${esc(l)}</text>`).join('');
    const labelSize=expanded?8.5:6.2;
