...
['MERCANTIL',brl.format(num(day.general))],['VENDA ELEGÍVEL',brl.format(num(day.eligible))],['SERVIÇOS',brl.format(services)],['NOTAS FISCAIS',String(num(day.invoiceCount))],['QTD. ELEGÍVEL',String(num(day.nfs))],['QTD. GARANTIAS',String(num(day.warrantyQty))],['CONVERSÃO',num(day.nfs)?efficiencyPct.format(conv):'—'],['EFICIÊNCIA',num(day.eligible)?efficiencyPct.format(eff):'—'],['COMISSÃO MERC.',brl.format(mercComm)],['COMISSÃO SERVIÇOS',brl.format(servComm)],['TOTAL COMISSÕES',brl.format(mercComm+servComm)]
    ...
    try {
      const historyCards=(history||[]).map((h)=>{
        const hs=h&&h.seller?h.seller:{};
        const a=hs.daily?sellerDailyAggregate(hs):{general:num(hs.general),services:num(h&&h.services),eligible:num(hs.eligible),nfs:num(hs.nfs),warrantyQty:num(hs.warrantyQty)};
        a.general=num(a.general)+num(hs.ecommerce);
        a.conversion=num(a.nfs)?num(a.warrantyQty)/num(a.nfs):0;
        a.efficiency=num(a.eligible)?num(a.services)/num(a.eligible):0;
        return sellerWorkspacePeriodCard(monthLabel((h&&h.month)||db.month),a,num(hs.assignedGoal),num(hs.serviceGoal));
      }).join('');
      compiled=`<div class="seller-finance-dashboard compiled-finance"><div class="seller-finance-main"><span>💵 PROJEÇÃO FINANCEIRA ATUAL</span><strong>${brl.format(financial.projectedTotal)}</strong><small>Inclui comissões informadas e DSR estimado</small></div></div><div class="seller-workspace-grid" style="margin-top:12px">${historyCards||'<div class="empty">Ainda não há histórico de outras competências.</div>'}</div>`;
    } catch (compiledError) {
