(() => {
  ...
  function dashboardSvg(values, labels, metric, type='bar') {
    const W=760,H=280,left=68,right=18,top=16,bottom=34,maxValue=Math.max(...values,0),axisMax=maxValue>0?maxValue*1.1:1;
    ...
    const axisLabels=labels.map((label,i)=>i%step===0?`<text x="${x(i)}" y="${H-10}" text-anchor="middle" fill="#718096" font-size="10">${esc(label)}</text>`:'').join('');
    let marks='';
    if(type==='line'){
  ...
  function branchDashboardMarkup(period='month', compare='none') {
    const metrics = [['merc','💰 Venda mercantil'], ['services','🛡️ Serviços'], ['conversion','🎯 Conversão'], ['efficiency','⚡ Eficiência'], ['ticket','🧾 Ticket médio']];
    if (period === 'year') {
