(() => {
  ...
  function dashboardDualCompareSvg(rows, labels, expanded=false){
    ...
    const W=expanded?Math.max(1500,Math.min(3600,260+rows.length*118)):780,H=expanded?560:340,left=78,right=78,top=74,bottom=52,plotH=H-top-bottom;
    ...
    const grid=gridPos.map((yy,i)=>{
      ...
      return `<line x1="${left}" y1="${yy}" x2="${W-right}" y2="${yy}" stroke="#e7edf4"/><text x="8" y="${yy+4}" fill="#718096" font-size="10" class="dual-axis-left">${dashboardAxisFormat(leftVal,'merc')}</text><text x="${W-right+8}" y="${yy+4}" fill="#718096" font-size="10" class="dual-axis-right">${Number(rightVal).toFixed(1).replace('.',',')}%</text>`;
    }).join('');
    const step=expanded?1:Math.max(1,Math.ceil(labels.length/8));
    ...
    const bars=rows.map(([,d],i)=>{
      ...
      return values.map((v,j)=>{
        ...
        return `<defs><linearGradient id="${gid}" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="${colors[j][0]}"/><stop offset="1" stop-color="${colors[j][1]}"/></linearGradient></defs><rect x="${xx}" y="${yy}" width="${bw}" height="${hh}" rx="4" fill="url(#${gid})"/><text x="${xx+bw/2}" y="${labelY}" text-anchor="middle" fill="${tone}" font-size="${expanded?6.4:5.2}" font-weight="900" class="dual-value-label ${j===0?'merc':'eff'}" style="paint-order:stroke;stroke:#fff;stroke-width:${expanded?2.1:1.4}px;stroke-linejoin:round">${txt}</text>`;
      }).join('');
    }).join('');
  ...
  function dashboardMercEfficiencyTrendSvg(rows, labels, expanded=false){
    ...
    const W=expanded?Math.max(1500,Math.min(3600,260+rows.length*118)):780,H=expanded?560:340,left=72,right=72,top=78,bottom=52;
    ...
    const labelSize=expanded?6.4:5.1;
    const mp=merc.map((v,i)=>{const yy=ym(v),offset=expanded?(i%2?28:13):10,ty=yy<top+offset+4?yy+16:yy-offset;return `<circle cx="${x(i)}" cy="${yy}" r="3.5" fill="#1688ec"><title>${esc(labels[i])} • Mercantil ${brl.format(v)}</title></circle><text x="${x(i)}" y="${ty}" text-anchor="middle" fill="#1268bd" font-size="${labelSize}" font-weight="800" class="dual-value-label merc" style="paint-order:stroke;stroke:#fff;stroke-width:${expanded?2.1:1.4}px;stroke-linejoin:round">${brl.format(v)}</text>`}).join('');
    const ep=eff.map((v,i)=>{const yy=ye(v),offset=expanded?(i%2?16:31):16,ty=yy>H-bottom-offset-4?yy-14:yy+offset;return `<circle cx="${x(i)}" cy="${yy}" r="3.5" fill="#f59e0b"><title>${esc(labels[i])} • Eficiência ${v.toFixed(2).replace('.',',')}%</title></circle><text x="${x(i)}" y="${ty}" text-anchor="middle" fill="#b66b00" font-size="${labelSize}" font-weight="800" class="dual-value-label eff" style="paint-order:stroke;stroke:#fff;stroke-width:${expanded?2.1:1.4}px;stroke-linejoin:round">${v.toFixed(2).replace('.',',')}%</text>`}).join('');
...
.dual-chart-legend{display:flex;flex-wrap:wrap;gap:14px;align-items:center;margin:8px 0 2px;color:#53677b;font-size:.76rem;font-weight:800}.dual-chart-legend span{display:inline-flex;align-items:center;gap:6px}.dual-chart-legend i{width:10px;height:10px;border-radius:3px;display:inline-block}.dual-chart-legend i.merc{background:#1688ec}.dual-chart-legend i.serv{background:#16a085}.dual-chart-legend i.eff{background:#f59e0b}.dual-chart-card{min-width:0}.dual-chart-card .svg-chart{min-height:250px}.dual-chart-card .dual-x-compact{display:none}.dashboard-card-expanded.dual-chart-card .dual-x-compact{display:block}.dashboard-card-expanded.dual-chart-card .svg-chart{min-width:1100px}.dashboard-card-expanded.dual-chart-card .svg-chart .dual-value-label{font-size:7.0px!important}.dashboard-card-expanded.dual-chart-card .svg-chart .dual-x-label,.dashboard-card-expanded.dual-chart-card .svg-chart .dual-axis-left,.dashboard-card-expanded.dual-chart-card .svg-chart .dual-axis-right{font-size:8.7px!important}.dashboard-card-expanded.dual-chart-card{overflow:auto!important}
