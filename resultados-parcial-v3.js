(() => {
  ...
  function metricCard(label, actualPct, targetPct, tone){
    const rate = targetPct > 0 ? actualPct / targetPct : 0;
    const width = Math.max(0, Math.min(100, rate * 100));
    const overflow = rate > 1 ? Math.min(30, (rate - 1) * 30) : 0;
    const neg = actualPct < 0;
    return `<div class="team-metric team-metric-${tone}${neg?' is-negative':''}">
      <span>${label}</span>
      <strong>${pctText(actualPct, actualPct && Math.abs(actualPct)<10 ? 1 : 0)}</strong>
      <div class="team-metric-track"><i style="width:${width}%"></i><b style="left:${Math.min(100,Math.max(0,targetPct>0?100:0))}%"></b>${overflow?`<em style="width:${overflow}%"></em>`:''}</div>
      <small>Meta: ${pctText(targetPct, targetPct<10?0:0)}</small>
    </div>`;
  }
  ...
  async function downloadComposite(){
    ...
    try{
      ...
      wrap.style.cssText='position:fixed;left:-12000px;top:0;width:1600px;background:#edf5ff;padding:28px;z-index:-1;box-sizing:border-box';
      ...
      const canvas=await html2canvas(wrap,{scale:3,backgroundColor:'#edf5ff',useCORS:true,logging:false,windowWidth:1600});
      wrap.remove();
